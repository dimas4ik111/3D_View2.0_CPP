...
//  vbo.allocate(rawObjData.array_of_v, rawObjData.num_of_v * sizeof(GLfloat));
  ...
  // z) GL_FLOAT - указываем тип данных атрибута (т.е. float) GL_FALSE -
  // Указываем, хотим ли мы, чтобы наши данные были нормализованы (актуально
  // только для целочисленных данных) 3 * sizeof(GLfloat) - шаг, на который
  // необходимо сдвигаться в пространстве атрибутов, чтобы получить значения
  // следующе вершины nullptr - Смещение от начала буфера, с которого начинается
  // обработка. Тут задано смещение 0, поскольку данные находятся в начале
  // массива
  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(GLfloat), nullptr);
  // Устанавливаем индекс атрибута (включаем). Если это не сделать, то атрибут
...
// Функция initializeGL() вызывается один раз, перед вызовом paintGL(). Здесь
// выполняется настройка контекста отображения.
void GLWidget::initializeGL() {
  initializeOpenGLFunctions();

  m_program = new QOpenGLShaderProgram;
  // Настраиваем вершинный шейдер (программы, выполняемые видеочипами, которые
  // производят математические операции с вершинами)
  m_program->addShaderFromSourceCode(QOpenGLShader::Vertex,
                                     vertexShaderSourceCore);
  // Настраиваем фрагментный шейдер (программы, выполняемые видеочипами, которые
  // вычисляют выходной цвет пикселей)
...
// Функция paintGL() вызывается всякий раз, когда возникает необходимость
// перерисовать содержимое виджета.
    ...
    if (rotateMode == 0) {
      m_program->setUniformValue(
          m_coeffMatrixLoc, projectionMatrix * cameraMatrix * rotateMatrix *
                                moveMatrix * scaleMatrix);
    } else {
      m_program->setUniformValue(m_coeffMatrixLoc,
                                 projectionMatrix * cameraMatrix * moveMatrix *
...
// Функция resizeGL() вызывается один раз, перед paintGL(), но после того, как
// будет вызвана функция initializeGL(). Здесь настраивается область просмотра
// (viewport), проекция и прочие настройки, которые зависят от размера виджета.
